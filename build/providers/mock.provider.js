...
class MockFingerprintProvider {
    ...
    checkStatus() {
        if (this.activeScenario === 'DEVICE_DISCONNECTED') {
            throw new provider_interface_1.FingerprintError('FINGERPRINT_DEVICE_NOT_FOUND', 'Mock device is disconnected');
        }
        if (this.activeScenario === 'DEVICE_BUSY') {
            throw new provider_interface_1.FingerprintError('FINGERPRINT_DEVICE_BUSY', 'Mock device is busy');
        }
    }
    ...
    async enrollStart(employeeId, fingerNumber) {
        ...
        if (this.activeScenario === 'CAPTURE_FAILED') {
            throw new provider_interface_1.FingerprintError('FINGERPRINT_CAPTURE_FAILED', 'Mock capture failed');
        }
    ...
    async enrollComplete(employeeId, fingerNumber) {
        ...
        if (this.activeScenario === 'ENROLLMENT_FAILED') {
            throw new provider_interface_1.FingerprintError('FINGERPRINT_CAPTURE_FAILED', 'Mock enrollment failed');
        }
        if (this.activeScenario === 'TIMEOUT') {
            throw new provider_interface_1.FingerprintError('FINGERPRINT_CAPTURE_TIMEOUT', 'Mock capture timeout');
        }
